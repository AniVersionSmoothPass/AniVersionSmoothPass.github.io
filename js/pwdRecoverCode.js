var recoveryMechanism = (function() {
	...
	//looks like its splitting up hash
	...
	function compareHashToExistingOnes (hashResult) {
		...
		// getting all hashes, putting it in a list
		...
		for (var i=0; i<storedHashes.length; i++) {
			...
			//doing some weird splitting thing
...
/* Guess: This function is called initially after the memory game is played and computes the hashes of
 all subsets of size 6 so that we can use them for the recovery mechanism later.

 Guess: groupFullList is a list of all of the PAO stories that the user memorized 

Guess: gpIndex is the number of times the user has played memory game previously
 */
...
/* Note: We are using one salt value for all of the hashes. This improves efficiency later */
		try {
			salt = localBCrypt.gensalt(round);
		} catch (err) {
		...
		//????????
		/* Guess: Set the salt value in the table for storage on dropbox */
		...
		/* If the user memorized < 6 stories then the password recovery mechanism would not be secure,
		   because any hash we generate could be cracked */
		if (groupFullList.length > MINIMUM_STORY_COUNT) {
			var k = MINIMUM_STORY_COUNT + 1;
			/* We eventually want to replace this with a shorter list */
			...
			for (var i=0; i<allCombinations.length; i++) {
				groupStr = ((allCombinations[i]).map( 
						function (l) {
							// Guess: This gets the action string
							var act = l[ACTION_INDEX_PRI];
							var obj = l[OBJECT_INDEX_PRI];
							// Guess: This is a deterministic map to prevent strings from getting too long
    ...
    /* Guess: this function is called after the user has enetered five stories in recovery mechanism page*/
	function gatherUserInput () {
		...
		for (var i=0; i<appConstants.getActionsList().length; i++) {
			...
			for (var j=0; j<appConstants.getObjectsList().length; j++) {
				...
				// groupGuess is doing the brute force
